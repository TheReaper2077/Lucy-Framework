...
void lf::TextureRect(Mesh *mesh, Texture *tex, float v0x, float v0y, float v1x, float v1y, const Vec2& uv0, const Vec2& uv1) {
	assert(mesh->layout == lf::Layout::Vertex3D);
	...
	if (mesh->type == lf::RenderType::TRIANGLES) {
		...
		mesh->vertices.emplace_back(Vec3(v0x, v0y));
		mesh->vertices.emplace_back(Vec3(uv0.x, uv0.y));
		mesh->vertices.emplace_back(Vec3(v0x, v1y));
		mesh->vertices.emplace_back(Vec3(uv0.x, uv1.y));
		mesh->vertices.emplace_back(Vec3(v1x, v1y));
		mesh->vertices.emplace_back(Vec3(uv1.x, uv1.y));
		mesh->vertices.emplace_back(Vec3(v1x, v1y));
		mesh->vertices.emplace_back(Vec3(uv1.x, uv1.y));
		mesh->vertices.emplace_back(Vec3(v1x, v0y));
		mesh->vertices.emplace_back(Vec3(uv1.x, uv0.y));
		mesh->vertices.emplace_back(Vec3(v0x, v0y));
		mesh->vertices.emplace_back(Vec3(uv0.x, uv0.y));
...
void lf::TextureRect(Mesh *mesh, Texture *tex, const Vec3& pos, const Vec3& scale, const Vec2& uv0, const Vec2& uv1) {
	lf::TextureRect(mesh, tex, pos.x, pos.y, pos.x + scale.x, pos.y + scale.y, uv0, uv1);
}
