...
// 	Vec2 initpos = Vec2(initposmouse.x/size, initposmouse.y/size);
// 	Vec2 finalpos = initpos + dir*10;
// 	Vec2 raylen;
// 	Vec2 voxel = Vec2(int(initpos.x), int(initpos.y));

// 	if (!stepdir.x) {
// 		raylen.x = (voxel.x + 1.0 - initpos.x) * stepsize.x;
// 	} else {
// 		raylen.x = (initpos.x - voxel.x) * stepsize.x;
// 	}
// 	if (stepdir.y) {
// 		raylen.y = (voxel.y + 1.0 - initpos.y) * stepsize.y;
// 	} else {
// 		raylen.y = (initpos.y - voxel.y) * stepsize.y;
// 	}
...
// 		lf::RenderRect(mesh, Vec3(voxel.x, voxel.y)*size, Vec3(size, size), Vec3(0, 1, 0), true);
// 	}

// 	std::cout << initpos.GetString() << ' ' << (int)initpos.x << '\n';
